...
/* local variables */
#define MAXCOUNT 30
int test_count = 5000, outer_count = 30;
int test_count1 = 10, test_count2 = 10, test_count3 = 10;
TickType_t t_start = 0, t_end = 0, t_total = 0;
/*------------------------------------------------*/

/* handlers */
TaskHandle_t TaskHandle_bubblesort_1, TaskHandle_bubblesort_2, TaskHandle_bubblesort_3;
// TaskHandle_t TaskHandle_bubblesort;
// TaskHandle_t TaskHandle_dijkstra;
// TaskHandle_t TaskHandle_idle;
TaskHandle_t TaskHandle_beebs;
/*----------------------------------------------------------*/
...
int main(void)
{
    ...
    // time_measurement_s(22);
    t_start = ARM_CM_DWT_CYCCNT = 0;
    ARM_CM_DWT_EXCCNT = 0;
...
void bench_result() /* FREERTOS_SYSTEM_CALL */
{
    // TickType_t time;
    portRAISE_PRIVILEGE();
    // time = ARM_CM_DWT_CYCCNT;
    // time = MPU_xTaskGetTickCount() - t_start;
    time_measurement_s(ARM_CM_DWT_CYCCNT + ARM_CM_DWT_EXCCNT);
    portRESET_PRIVILEGE();

    // test_count1 = test_count2 = test_count3 = 10;
    // t_start = ARM_CM_DWT_CYCCNT = 0;
}

#ifdef MICRO

static void sherlocMicroTask1(void *pvParameters)
{
    ...
    for (; count > 0; count--)
    {
        ...
        {
            // if i is the multiple of 10, then yield
            if (i % 100 == 0)
            {
                portYIELD();
            }
...
static void sherlocMicroTask2(void *pvParameters)
{
    // configPRINTF(("Started Microbenchmark \r\n"));
    int iterations = 1000;
    int i = 0;
...
void start_microbenchmark()
{
    ...
    TaskParameters_t microTaskParameters1 =
        {
            .pvTaskCode = sherlocMicroTask1,                              // pvTaskCode - the function that implements the task.
            .pcName = "MicroBenchmark1",                                  // pcName - just a text name for the task to assist debugging.
            ...
            .puxStackBuffer = microStackBuffer1,                          // puxStackBuffer - the buffer to be used as the task stack.

            // xRegions - Allocate up to three separate memory regions for access by
            .xRegions =
                {
                    {&microStackBuffer1[0], BufferSize, portMPU_REGION_READ_WRITE}, // the other two region left unused.
                    {0, 0, 0},
                    {0, 0, 0}}};

    if (xTaskCreateRestricted(&microTaskParameters1, NULL) == pdPASS)
    {
        xReturn = pdPASS;
    }
    else
    ...
    TaskParameters_t microTaskParameters2 =
        {
            .pvTaskCode = sherlocMicroTask2,                              // pvTaskCode - the function that implements the task.
            .pcName = "MicroBenchmark2",                                  // pcName - just a text name for the task to assist debugging.
            .usStackDepth = configMINIMAL_STACK_SIZE,                     // usStackDepth	- the stack size DEFINED IN WORDS.
            .pvParameters = NULL,                                         // pvParameters - passed into the task function as the function parameters.
            .uxPriority = (configMAX_PRIORITIES - 3 | portPRIVILEGE_BIT), // uxPriority - task priority, set the portPRIVILEGE_BIT if the task should run in a privileged state.
                                                                          //.uxPriority = configMAX_PRIORITIES - 3, // uxPriority - task priority, set the portPRIVILEGE_BIT if the task should run in a privileged state.
            .puxStackBuffer = microStackBuffer2,                          // puxStackBuffer - the buffer to be used as the task stack.

            // xRegions - Allocate up to three separate memory regions for access by
            // the task, with appropriate access permissions.
            .xRegions =
                {
                    {&microStackBuffer2[0], BufferSize, portMPU_REGION_READ_WRITE}, // the other two region left unused.
                    {0, 0, 0},
                    {0, 0, 0}}};
    if (xTaskCreateRestricted(&microTaskParameters2, NULL) == pdPASS)
    {
        xReturn = pdPASS;
    }
    else
...
static void sherlocBeebsTask(void *pvParameters)
{
    ...
    while (local_count--)
    {
        ...
        {
            // bubblesort: {0x28204980, 0x1a0, portMPU_REGION_READ_WRITE},
